...
set exitcode=0
set buildmode=clean
set DLL=PositionControllerTilt.dll
...
ECHO ------------------------------------------------------------
ECHO 20-sim Dynamic DLL for 'PositionControllerTilt'
ECHO ------------------------------------------------------------
...
IF DEFINED VSVARS32 (
	...
	IF DEFINED DEVENV (
		IF NOT EXIST %DEVENV% (
			...
			echo   PositionControllerTilt.emx
			goto END
		)
	)
)

set OPTS_DLL="%PROJ_DIR%\PositionControllerTilt.sln" /build "%buildconfig%"
set CLEAN_DLL="%PROJ_DIR%\PositionControllerTilt.sln" /clean "%buildconfig%"
...
:COMPILE_DLL
  IF %DEVENV% NEQ "" (
	...
	%DEVENV% "%PROJ_DIR%\PositionControllerTilt.sln" /clean "%buildconfig%"
    ECHO Compiling 20-sim Dynamic DLL for submodel "PositionControllerTilt"
    %DEVENV% "%PROJ_DIR%\PositionControllerTilt.sln" /build "%buildconfig%"
  ) ELSE (
	ECHO Compiling 20-sim Dynamic DLL for submodel "PositionControllerTilt"
    msbuild.exe "%PROJ_DIR%\PositionControllerTilt.vcxproj" /p:Configuration=Release /t:Rebuild /verbosity:minimal
  )
  
  IF NOT EXIST ..\%DLL% (
  	set DIETEXT="PositionControllerTilt.dll failed to build!  See ..\%PROJ_DIR%\%buildconfig%\BuildLog.htm for details."
  	goto DIE
  )
  ECHO Done.
  ECHO ------------------------------------------------------------
...
:COMPILE_NO_CLEAN_DLL
  IF %DEVENV% NEQ "" (
	...
	ECHO Compiling 20-sim Dynamic DLL for submodel "PositionControllerTilt"
    msbuild.exe "%PROJ_DIR%\PositionControllerTilt.vcxproj" /p:Configuration=Release /t:Build /verbosity:minimal
  )
...
:MAKE_BUILD_DLL
  cd ..
  ECHO Your PositionControllerTilt Dynamic DLL is ready and can be found in:
  ECHO   %CD%
