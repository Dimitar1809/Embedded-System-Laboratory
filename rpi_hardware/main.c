...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < loops; i++) {
    ...
    uint16_t yaw_value = (uint16_t)(rx_word & 0x0000FFFF);         // Mask lower 16 bits
    uint16_t pitch_value = (uint16_t)((rx_word >> 16) & 0x0000FFFF); // Shift right by 16, then mask

    // print as unsigned decimal
    printf("yaw: %u, pitch: %u\n", yaw_value, pitch_value);
  }
